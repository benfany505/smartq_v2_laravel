            ...
            <div class="col-sm-4" style="color: white">
                ...
                <form id="form_layanan">
                    ...
                    <div class="form-group row" style="margin: 5px 2px 0px 2px;">
                        <label for="kode_layanan" class="col-sm-4 col-form-label ">Kode</label>
                        <div class="col-sm-8">
                            <Select class="form-control form-control-sm" id="kode_layanan" name="kode_layanan" required
                                style="background-color: transparent; color: white">
                                @php
                                    // create array ok kode for each $layanan->kode
                                    ...
                                    // create option in select A to Z except kode in array
                                    for ($i = 65; $i <= 90; $i++) {
                                        $char = chr($i);
                                        if (!in_array($char, $kode)) {
                                            echo '<option value="' . $char . '" style="background-color: #222F3E; ">' . $char . '</option>';
                                        }
                                    }
                    ...
                    <div class="form-group row" style="margin: 5px 2px 0px 2px;">
                        <label for="nama_layanan" class="col-sm-4 col-form-label ">Nama</label>
                        <div class="col-sm-8">
                            <input name="nama_layanan" id="nama_layanan" class="form-control form-control-sm" type="text"
                    ...
                    <div class="form-group row" style="margin: 5px 2px 0px 2px;">
                        <label for="status_layanan" class="col-sm-4 col-form-label ">Status</label>
                        <div class="col-sm-8">
                            <Select class="form-control form-control-sm" id="status_layanan" name="status_layanan" required
                                style="background-color: transparent; color: white">
                ...
                <table class="table table-sm  table-striped  mt-1" id="table_layanan">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" width="10%" class="text-left">Kode</th>
                            <th scope="col" width="40%" class="text-left">Nama Layanan</th>
...
@push('custom-js')
    ...
    <script>
        ...
        const myForm = document.getElementById('form_layanan');
        const kode_layanan = document.getElementById('kode_layanan');
        const nama_layanan = document.getElementById('nama_layanan');
        const status_layanan = document.getElementById('status_layanan');
        ...
        myForm.addEventListener('submit', function(e) {
            e.preventDefault();
            var myJson = {
                kode: kode_layanan.value.toUpperCase(),
                nama: nama_layanan.value.toUpperCase(),
                status: status_layanan.value,
                // get session requestorEmail
                requestorUsername: '{{ session()->get('username') }}',

            };
            myForm.method = 'POST';
            myForm.action = '/api/layanan';
        ...
        $(document).on('click', '#save', function() {
            // get row data
            var currentRow = $(this).closest("tr");
            var kode = currentRow.find("td:eq(0)").find("input[type=text]").val();
            ...
            currentRow.find("td:eq(0)").html(kode);
        ...
        $(document).on('click', '#hapus', function() {
            ...
            var r = confirm("Are you sure want to delete " + kode + " - " + nama + " - " + status + "?");
            if (r == true) {
                ...
                const xhr = new XMLHttpRequest();
                // delete based on id
                xhr.open('DELETE', '/api/layanan/' + id);
